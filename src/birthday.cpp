...
void handlebirthday() {
  if (server.method() == HTTP_POST) {
    ...
    String html = String(FPSTR(htmlhead));
    html += "<div class='card'><p>Die Tage wurden gespeichert.</p></div></main></body></html>";
    ...
    String body = "<div class='card'><p>Hier kannst du Tage eingeben, an denen die Buchstaben zufaellige Farben bekommen, z.B. an Geburtstagen.</p></div>";
    body += "<form action='/birthday' method='POST'><div class='card'>";
    body += "<label for='countdown'>Countdown (Datum & Zeit)</label>";
    body += "<input type='datetime-local' id='countdown' name='countdown' value='" + countdownVal + "'>";
    body += "<br/><small>Wenn gesetzt, werden die letzten 99 Sekunden als Ziffern angezeigt.</small><br/>";
    ...
    for (int i = 0; i < 5; i++) {
      ...
      body += "<input type='checkbox' name='delete" + String(i) + "' value='1'> loeschen<br>";
    }

    body += "<button type='submit'>Absenden</button></div></form></main></body></html>";

    String html = String(FPSTR(htmlhead)) + body;
    server.send(200, "text/html", html);
  }
}
